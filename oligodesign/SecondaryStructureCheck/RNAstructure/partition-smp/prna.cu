...
DEV HOST inline static int can_pair(int i, int j, int n, const base_t *s, const int *bcp)
{
  if (!bcp[i] || !bcp[j])
    return 0;
  if (j < i) {
...
DEV HOST static real_t *array_val(real_t *a, int i, int j, int n, const base_t *s, const int *bcp)
{
  return can_pair(i,j,n,s,bcp) ? &a[ind(i,j,n)] : 0;
}
...
(int d, int n, base_t *s, int *bcp, real_t *v, real_t *x, real_t *w5, real_t *w3, param_t p)
{
  ...
  for (i = ISTART; i < n; i += IINC) {
    ...
    if ((is_exterior(i,j) && i-j <= LOOP_MIN) || !can_pair(i,j,n,s,bcp))
      continue;
    real_t vij = INF;
    if (i != n-1 && j != 0) {
      /* hairpin loop */
      ...
      /* stack */
      if (can_pair(i+1,j-1,n,s,bcp) && !(is_interior(i,j) && d <= LOOP_MIN-2))
	...
	if (k != n-1) {
	  if ((v1 = array_val(v,i+1,k,n,s,bcp)))
	    free_energy_accumulate(&vij, coaxial_flush(s,j,i,i+1,k,p) + (*v1) + p->a_2c + 
				   y[ind(k+1,j-1,n)]);
	  if (ktmp+2 < jtmp-1 && i+1 != n-1 && k+1 != n-1 && (v1 = array_val(v,i+2,k,n,s,bcp))) {
	...
	if (k != 0) {
	  if ((v1 = array_val(v,k,j-1,n,s,bcp)))
	    free_energy_accumulate(&vij, coaxial_flush(s,k,j-1,j,i,p) + (*v1) + p->a_2c + 
				   y[ind(i+1,k-1,n)]);
	  if (j != 1 && ktmp > i+3 && (v1 = array_val(v,k,j-2,n,s,bcp))) {
	    const real_t tmp = (*v1) + p->a_2b_2c;
	    if (k != 1)
 ...
 param_t p) 
{
  ...
  for (i = ISTART; i < n; i += IINC) {
	...
	continue;
      real_t *v1, *v2;
      if ((v1 = array_val(v,i,k,n,s,bcp)) && (v2 = array_val(v,k+1,j,n,s,bcp)))
	free_energy_accumulate(&tmp1, (*v1) + (*v2) + coaxial_flush(s,i,k,k+1,j,p));
      if (j == 0 || k+1 == n-1)
	continue;
      if (i != n-1 && (v1 = array_val(v,i+1,k,n,s,bcp)) && (v2 = array_val(v,k+2,j,n,s,bcp)))
	free_energy_accumulate(&tmp2, (*v1) + (*v2) + coaxial_mismatch1(s,i+1,k,k+2,j,p));
      if ((v1 = array_val(v,i,k,n,s,bcp)) && (v2 = array_val(v,k+2,j-1,n,s,bcp)))
...
prna_t prna_new(const char *s, param_t par, int quiet)
{
  ...
  sequence_from_string(p->seq, p->base_can_pair, s);
...
#define ALLOC(a,sz) CU(cudaMalloc(&a,(sz)*sizeof(real_t)))
  ...
  CU(cudaMemcpy(dev_bcp, p->base_can_pair, n*sizeof(int), cudaMemcpyHostToDevice));
...
#define SHOWARR(a)	 \
  if (p->a) {		      \
    ...
    for (i = 0; i < n; i++) { \
      printf("%s%4d: ",#a,i+1);				\
      for (j = 0; j < n; j++) {				\
	const real_t *aij = array_val(p->a,i,j,n,s,bcp);	\
...
static real_t free_energy_of_pair(const prna_t p, int i, int j)
{
  ...
  const base_t *s = p->seq;
  const int *bcp = p->base_can_pair;
  if (can_pair(i,j,n,s,bcp))
    return *array_val(p->v,i,j,n,s,bcp) + *array_val(p->v,j,i,n,s,bcp) - p->w3[0];
  else
    return INF;
}
...
void prna_write_neg_log10_probabilities(const prna_t p, const char *fn)
{
  ...
  for (i = 0; i < p->n; i++)
    for (j = i+1; j < p->n; j++)
      if (can_pair(i,j,p->n,p->seq,p->base_can_pair))
	fprintf(f,"%-8d%-8d"RF"\n", i+1, j+1,
...
void prna_write_probability_matrix(const prna_t p, const char *fn)
{
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++)
      fprintf(f,RF" ", 
	      can_pair(i,j,n,s,bcp) ? probability_of_pair(p,i,j) : 0);
    fprintf(f,"\n");
  }
  fclose(f);
}
