...
void RemovePseudoknots::run() {
	...
	if( error == 0 ) {
		...
		if (writeBracket)
			error = strand->WriteDotBracket(output.c_str(), -1, DBN_FMT_MULTI_TITLE, NO_CT_ENERGY_COMMENTS);
		else
			error = strand->WriteCt( output.c_str(), false, NO_CT_ENERGY_COMMENTS); // pass in "" for the energyLabel so that energies will NOT be written to the CT file.
