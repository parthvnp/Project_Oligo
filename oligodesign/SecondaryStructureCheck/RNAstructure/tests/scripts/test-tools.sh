...
# initRef: Initialize variables to refer to another test.
...
# Example:    initRef 'without_options' _MAIN
#    Initializes the following variables:
#      TEST_MAIN,  OUT_MAIN,  OKBASE_MAIN,  OK_MAIN,  STDO_MAIN,  etc.
...
function endTest() {
    ...
    ((keep)) || rm -f ./{"$TEST","$TBASE"}*  # remove all test-related files   (perhaps use ./{"$TEST","$TBASE"}{.,_}* to narrow results)
...
function runProgAndDiff() {
    ...
    # Perform post-processing. This includes text-replacements (from --replace) as well as
    # additional custom post-processing if the TEST_POST_PROCESS function is defined.
    local ppexit=0
    fixParams "$diff1"; local outfile="${FIXED_PARAMS[0]}"
...
# Replace parameters that start with a special placeholder
function fixParams() {
    # e.g. @TEST_sav.txt  ==>  ${TEST}_sav.txt ==> Fold_dna_option_sav.txt
    FIXED_PARAMS=()
    local replace=(TBASE TESTNAME TEST STDO OUTFILE OUT OKFILE OKDIR OKBASE OK INDIR EXENAME EXEBASE EXE ERRO ERRDIR)  # This list MUST be kept in reverse ABC order (to prevent e.g. @OKFILE from being replaced with @{OK}FILE etc.)
    for arg; do
        case "$arg" in
            @ERRDIR) arg="${ERROR_OUTPUT_DIR:-${EXENAME}_OUTPUT}" ;;
            *)  # all others, test for placeholder replacements
                for rep in "${replace[@]}"; do
                  if [[ $arg == *@$rep* ]]; then
                    #arg="${!rep}${arg:${#rep}+1}"  # replace the placeholder, but keep the rest of arg intact.
                    arg="${arg//"@$rep"/${!rep}}"  # replace the placeholder, but keep the rest of arg intact.  (for IDE:")
                    #break  # exit the rep loop
                  fi
                done
            ;;
        esac
...
function replaceVarsInText() {
    local text="$1"
    local list=(TESTNAME TEST TBASE STDO OUTFILE OUT OKFILE OKDIR OKBASE OK INDIR EXE ERRO ERRDIR)  # This list MUST be kept in reverse ABC order (to prevent e.g. @OKFILE from being replaced with @{OK}FILE etc.)
    local name rep
    for name in "${list[@]}"; do
        case $name in
            ERRDIR) rep="${ERROR_OUTPUT_DIR:-${EXENAME}_OUTPUT}" ;;
            *) rep="${!name}" ;;
        esac
        text="${text//@"$name"/$rep}"
    done
...
################################################################################
# Performs a diff on the output of a test. Redirects the diff output to the file ${TEST}_diff.txt
# Usage runDiff  [FILE1] [FILE2]
# If not specified: FILE1 is assumed to be {TEST}_output$EXT 
#                   FILE2 is assumed to be $OKDIR/${TEST}_OK$EXT
#     (Where $EXT and $OKDIR should be set at the top of the script, or at least before runDiff is called.) 
################################################################################
function runDiff() {
    shouldIncludeTest || return
    ...
    local f1=${1:-$OUT$EXT} f2=${2:-$OK$EXT} fo
    ...
    local f2_OS="${f2/_OK/_@${OS}_OK}"; [[ -f $f2_OS ]] && f2="$f2_OS"

    if [[ $3 ]]; then fo="${TEST}_$3_diff.txt"; else fo="${TEST}_diff.txt"; fi
    ...
    if [[ -f $fo ]]; then
        warnmsg 'The diff output file already exists: '"$fo"
        local pfx="${fo%_diff.txt}"  i=2
        while [[ -f $fo ]]; do fo=$pfx$((i++))_diff.txt; done
    fi
    ...
    logCmd runDiff "$3" "$DIFF_CMD" $DIFF_FLAGS "$f1"  "$f2" '>&' "$fo" 

    if [[ ! -f $f1 ]]; then
        errmsg "      ERROR: Expected output file missing: $f1"
    elif [[ ! -f $f2 ]]; then
        errmsg "      ERROR: Expected OK-file missing: $f2"
    else
        "$DIFF_CMD" $DIFF_FLAGS "$f1"  "$f2" >& "$fo" 
    fi
}
